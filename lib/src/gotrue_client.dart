...
class GoTrueClient {
  ...
  Future<GotrueSessionResponse> _recoverSession(String jsonStr) async {
    try {
  ...
  Future<GotrueSessionResponse> recoverSession() async {
    final String? persistSessionString =
        await _storage.read(Constants.defaultStorageKey);

    if (persistSessionString == null) {
      return GotrueSessionResponse(
          error: GotrueError('Missing local Session.'));
    }

    return _recoverSession(persistSessionString);
  }
  ...
  Future<void> _saveSession(Session session) async {
    ...
    if (authPersistence == AuthPersistence.local) {
      await _storage.write(
          Constants.defaultStorageKey, session.persistSessionString);
    }
